...
describe("canary-sign-in", () => {
  ...
  beforeEach(() => {
    ...
    delete require.cache[require.resolve("../src/canary-sign-in")];
    handler = require("../src/canary-sign-in").handler;
  });
  ...
  it("should start client on port 3031", async () => {
    await handler();
  ...
  it("should empty OTP bucket before running steps", async () => {
    await handler();
  ...
  it("should call steps in correct order", async () => {
    await handler();
    expect(stepsStubs.launchClient).to.have.been.called;
    expect(stepsStubs.clickSignIn).to.have.been.called;
    expect(stepsStubs.enterEmail).to.have.been.calledWith(
      pageStub,
      "user@example.com"
    );
    expect(stepsStubs.submitEmail).to.have.been.called;
    expect(stepsStubs.enterPassword).to.have.been.calledWith(
      pageStub,
      "Pa55word!"
    );
    expect(stepsStubs.submitPassword).to.have.been.called;
    expect(stepsStubs.enterOtpCode).to.have.been.calledWith(
      pageStub,
      "07700900000",
      "otp-bucket"
    );
    expect(stepsStubs.submitOtpCode).to.have.been.called;
